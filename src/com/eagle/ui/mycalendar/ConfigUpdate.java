...
public class ConfigUpdate extends JDialog implements ActionListener, KeyListener {
	private static final long serialVersionUID = 1L;
	private CalendarAction mmrAction = new CalendarAction();
	
	private JPanel clkPanle ;
	private JLabel label_0=new JLabel("修改纪念日配置",JLabel.CENTER);
	...
	public ConfigUpdate(JFrame _f, MmrDayData data){
		super(_f, "修改纪念日配置", true);
	...
	private void initBody(MmrDayData data){
		initComb(data);
	...
	public void actionPerformed(ActionEvent event){
		try{
			if("add".equals(event.getActionCommand())){
				...
				if(mmrAction.updateMmyDay(data)){
					this.setVisible(false);
					this.dispose();
				}else{
					JOptionPane.showMessageDialog(this, "修改失败，可能是纪念日时间已设置！");
				}
			}else{
				this.setVisible(false);
				this.dispose();
	...
	public void initComb(MmrDayData data){
		txt_10.addItem("");
		...
		String dateArr[] = data.getId().split("-");
		if(dateArr.length==3){
			txt_10.setSelectedItem(dateArr[0]);
			txt_11.setSelectedItem(dateArr[1]);
			txt_12.setSelectedItem(dateArr[2]);
		}else{
			txt_11.setSelectedItem(dateArr[0]);
			txt_12.setSelectedItem(dateArr[1]);
		}
		txt_10.setEnabled(false);
		txt_11.setEnabled(false);
		txt_12.setEnabled(false);
		
		txt_2.setText(data.getContent());
		txt_3.setSelectedItem(data.getType());
		txt_4.setSelectedItem(data.getValid());
	}
}
