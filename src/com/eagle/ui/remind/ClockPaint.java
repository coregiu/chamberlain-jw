...
class ClockPaint extends JPanel implements Runnable {
	...
	public ClockPaint(int x, int y, int r, String content) {
	...
	public void paint(Graphics g) {
		...
		super.paint(g);
		//g.setColor(null);
		g.drawImage(ImageData.clockBg.getImage(), 0, 0, null);
		...
		x1 = (int) ((r - r / 3.5) * Math.sin(rad * m));
		y1 = (int) ((r - r / 3.5) * Math.cos(rad * m));
		g.drawLine(x + r, y + r, x + r + x1, y + r - y1);
		// 时针
		g.setColor(Color.YELLOW);
		x1 = (int) ((r - r / 2.0) * Math.sin(rad * h));
		y1 = (int) ((r - r / 2.0) * Math.cos(rad * h));
		g.drawLine(x + r, y + r, x + r + x1, y + r - y1);
		// 数字
		g.setColor(Color.BLACK);
		int d = 29;
		...
		if(StringUtil.isNotEmpty(content)){
			String tmp;
			int length = 23;
			for(int i=0;i<=content.length()/length&&content.length()<80;i++){
				if(i<content.length()/length){
					tmp = content.substring(i*length,(i+1)*length-1);
				}else{
					tmp = content.substring(i*length,content.length());
				}
				g.drawString(tmp, 10, (i+1)*20+300);
			}
		}
	}
