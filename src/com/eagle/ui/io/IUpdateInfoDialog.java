...
public class IUpdateInfoDialog extends JDialog implements ActionListener, KeyListener, MouseListener{
	private static final long serialVersionUID = 1L;

	private IOConfAction ioConfAction = new IOConfAction();
	
	private JLabel label_0=new JLabel("修改收入信息",JLabel.CENTER);
    private JLabel label_1=new JLabel("收入编号：",JLabel.CENTER);
    private JLabel label_2=new JLabel("收入名称：",JLabel.CENTER);
    private JLabel label_3=new JLabel("收入总金额：",JLabel.CENTER);
    private JLabel label_4=new JLabel("纯收入金额：",JLabel.CENTER);
    private JLabel label_5=new JLabel("收入类型：",JLabel.CENTER);
	...
	public IUpdateInfoDialog(JFrame _f, IODataBean _ioDataBean){
		super(_f, "修改收入信息", true);
	...
	private JComponent body(IODataBean ioDataBean){
        ...
        JPanel panel_4=new JPanel(new GridLayout(1,2,1,1));
        ...
        DataUtil.setIType(txt_5);
        txt_6.setLineWrap(true);
        txt_7.addMouseListener(this);
        
        txt_1.setText(ioDataBean.getIId());
        txt_1.setEditable(false);
        txt_2.setText(ioDataBean.getIName());
        txt_3.setText(ioDataBean.getIAllMoney());
        txt_4.setText(ioDataBean.getICleanMoney());
        txt_5.setSelectedItem(ioDataBean.getIType());
        txt_6.setText(ioDataBean.getIResume());
        txt_7.setText(ioDataBean.getIDate());
        txt_5.addItem("其它类型");
	...
	public void actionPerformed(ActionEvent event){
		if("add".equals(event.getActionCommand())){
			...
			try{
				...
				Double.parseDouble("".equals(txt_4.getText())?"0":txt_4.getText());
			}catch(Exception e){
				JOptionPane.showMessageDialog(this, "金额项必须填写正确的数字！");
				return;
			}
			IODataBean ioDataBean = new IODataBean();
			ioDataBean.setIId(txt_1.getText());
			ioDataBean.setIName(txt_2.getText());
			ioDataBean.setIAllMoney(txt_3.getText());
			ioDataBean.setICleanMoney("".equals(txt_4.getText())?txt_3.getText():txt_4.getText());
			ioDataBean.setIType((String)((txt_5.getSelectedItem()==null||"其它类型".equals(txt_5.getSelectedItem()))?("".equals(txt_8.getText())?"其它类型":txt_8.getText()):txt_5.getSelectedItem()));
			ioDataBean.setIResume(txt_6.getText());
			ioDataBean.setIDate(txt_7.getText());
			
			if(ioConfAction.updateIInfo(ioDataBean)){
				ConfBean.loadIOInfo();
