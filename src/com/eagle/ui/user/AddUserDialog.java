...
public class AddUserDialog extends JDialog implements ActionListener, KeyListener{
	private static final long serialVersionUID = 1L;

	private UserAction userAction = new UserAction();
	
	private JLabel label_0=new JLabel("添加用户",JLabel.CENTER);
    ...
    private MainFrame f;
	public AddUserDialog(MainFrame _f){
		super(_f, "添加用户", true);
		f = _f;
		this.add(body());
	...
	private JComponent body(){
        ...
        button_1.setIcon(ImageData.newUser);
	...
	public void actionPerformed(ActionEvent event){
		if("add".equals(event.getActionCommand())){
			...
			if(userAction.addUser(userBean)){
				ConfBean.loginUserInfo = userBean;
				ConfBean.reLoad();
				f.setMenuEnable(true);
				f.initContentPanel();
				this.setVisible(false);
				this.dispose();
			}else{
				JOptionPane.showMessageDialog(this, "添加失败，检查数据！");
			}
		}else{
			this.setVisible(false);
			this.dispose();
